...
function pintar_formulario_modificar_tipo_cita($tipo_cita, $precio, $tiempo_estimado, $opciones_tipos_cita) {
    $form = <<<FORMULARIO
    <html lang="es">
    <head>
        <meta charset="UTF-8">
    ...
    <body>
        <form action="modificar_servicio_cita.php" method="post">
            <h1>Modificar servicio para cita</h1>
...
if (empty($_POST)) {
    ...
    $query_tipos_cita = "SELECT cod_tipo_cita, tipo_cita FROM tipo_cita WHERE 1=1";
    $res = mysqli_query($conex, $query_tipos_cita);
         
         while ($fila = mysqli_fetch_array($res)){
             
             $opciones_tipos_cita .= "<option value='{$fila['tipo_cita']}'>{$fila['tipo_cita']}</option>";
        ...
        pintar_formulario_modificar_tipo_cita($tipo_cita, $precio, $tiempo_estimado, $opciones_tipos_cita);
    ...
    if (validar_datos_tipo_cita($tipo_cita, $precio, $tiempo_estimado)) {
            ...
            $query = "UPDATE tipo_cita SET tipo_cita='$tipo_cita', precio= '$precio', tiempo_estimado='$tiempo_estimado' WHERE tipo_cita='$tipo_cita'";
